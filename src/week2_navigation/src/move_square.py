...
class Square():
    ...
    def main_loop(self):
        while not self.ctrl_c:
            ...
            if(self.turn):
                if(abs(self.theta_z - self.theta_z0) >= pi/2): # if rotated 90 degrees relative to prev
                    self.x0 = self.x
                    self.y0 = self.y
                    self.vel.linear.x = 0.1
                ...
                if(sqrt((self.x0 - self.x)*(self.x0 - self.x) + (self.y0 - self.y)*(self.y0 - self.y)) >= 1): # euclidean distance travelled 1m relative to previous position
                    self.theta_z0 = self.theta_z
                    self.vel.linear.x = 0
